...
import { ValuationRequest, ValuationMethod, ValuationResult } from '@shared/api';
...
export class ValuationEngine {
  ...
  static async calculateValuation(data: ValuationRequest, userId: string): Promise<ValuationResult> {
    ...
    const weightedValuation = methodResults.reduce((sum, result) => 
      sum + (result.value * result.weight), 0) / totalWeight;
    ...
    const overallConfidence = methodResults.reduce((sum, result) => 
  ...
  private static async calculateMethodValue(methodName: string, data: ValuationRequest): Promise<number> {
    switch (methodName) {
      case 'Berkus Method':
  ...
  private static berkusMethod(data: ValuationRequest): number {
    ...
    if (data.industry === 'technology' || data.industry === 'saas') valuation += 250000;
  ...
  private static scorecardMethod(data: ValuationRequest): number {
    ...
    if (marketValue > 10000000000) baseValuation *= 1.3; // $10B+ market
  ...
  private static riskFactorMethod(data: ValuationRequest): number {
    ...
    const risks = [
      ...
      { factor: 'Stage of business', adjustment: data.stage === 'growth' ? -1 : data.stage === 'idea' ? 2 : 0 },
      { factor: 'Legislation/Political', adjustment: 0 }, // Neutral
      { factor: 'Manufacturing', adjustment: data.industry === 'technology' ? -1 : 0 },
      { factor: 'Sales channels', adjustment: data.revenue ? -1 : 1 },
      { factor: 'Funding/Capital', adjustment: data.fundingGoal > 5000000 ? 1 : 0 },
      { factor: 'Competition', adjustment: 1 }, // Assume competitive market
      { factor: 'Technology', adjustment: data.industry === 'technology' ? -1 : 0 },
      { factor: 'Litigation', adjustment: 0 }, // Neutral
      { factor: 'International', adjustment: data.geographicMarket === 'global' ? 1 : 0 },
      { factor: 'Reputation', adjustment: 0 }, // Neutral
      { factor: 'Lucidity/Focus', adjustment: data.description.length > 200 ? -1 : 0 }
    ];
  ...
  private static dcfMethod(data: ValuationRequest): number {
    ...
    const totalValue = cashFlows.reduce((sum, cf) => sum + cf, 0) + presentTerminalValue;
  ...
  private static comparableMethod(data: ValuationRequest): number {
    ...
    if (data.revenue && data.revenue > 0) {
      ...
      const marketPenetration = data.stage === 'growth' ? 0.001 : 
                               data.stage === 'early-revenue' ? 0.0005 : 0.0001;
      
      return marketValue * marketPenetration;
    }
  }

  private static calculateMethodWeight(method: ValuationMethod, data: ValuationRequest): number {
    let weight = 1.0;
  ...
  private static async saveValuationToDatabase(result: ValuationResult, data: ValuationRequest): Promise<void> {
    ...
    `, [
      result.id, result.userId, data.companyName, data.industry, data.stage, data.description,
      data.revenue, data.expenses, data.teamSize, data.marketSize, data.fundingGoal,
      data.businessModel, data.projectedRevenue, data.burnRate, data.runway,
      data.previousFunding, data.geographicMarket, data.competition, data.useOfFunds,
      result.estimatedValuation.min, result.estimatedValuation.max, result.estimatedValuation.primary,
      result.primaryMethod, result.confidence, JSON.stringify(result.methods), result.createdAt
    ]);
  }
  ...
  static async getValuationById(id: string, userId: string): Promise<ValuationResult | null> {
    const db = getDatabase();
