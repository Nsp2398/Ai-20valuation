...
export class AuthService {
  ...
  static async createUser(data: {
    ...
    try {
      await db.run(`
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `, [userId, data.email || null, data.phone || null, passwordHash, data.firstName, data.lastName]);
  ...
  static async storeVerificationCode(phone: string, code: string, userId?: string): Promise<string> {
    const db = getDatabase();
    const codeId = uuidv4();
