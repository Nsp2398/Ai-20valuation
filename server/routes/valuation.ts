...
export const createValuation: RequestHandler = async (
  ...
  try {
    ...
    const missingFields = requiredFields.filter(
      (field) => !data[field as keyof ValuationRequest],
    );
...
export const getUserValuations: RequestHandler = async (
  req: AuthenticatedRequest,
  res,
) => {
  try {
...
export const getValuationById: RequestHandler = async (
  req: AuthenticatedRequest,
  res,
) => {
  try {
...
export const getDashboard: RequestHandler = async (
  ...
  try {
    ...
    const recentValuations = await ValuationEngine.getUserValuations(
      req.user.id,
    );
    ...
    const averageValuation =
      totalValuations > 0
        ? recentValuations.reduce(
            (sum, v) => sum + v.estimatedValuation.primary,
            0,
          ) / totalValuations
        : 0;
    ...
    const portfolioGrowth =
      totalValuations > 1
        ? ((recentValuations[0].estimatedValuation.primary -
            recentValuations[recentValuations.length - 1].estimatedValuation
              .primary) /
            recentValuations[recentValuations.length - 1].estimatedValuation
              .primary) *
          100
...
export const deleteValuation: RequestHandler = async (
  req: AuthenticatedRequest,
  res,
) => {
  try {
